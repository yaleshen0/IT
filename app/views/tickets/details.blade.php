          ...
          <div class="navbar-header">
            ...
            <div class="row">
              ...
              <div class='chatContainer'>
                  <div class='chatHeader' value="{{$ticket['_id']}}">
                    <h3>Comments</h3>
                  </div>

                  <div class='chatMessages'>
                  @if(isset($comm['comment']))
                    @foreach($comm['comment'] as $key => $value)
                      <li class="cm">{{$value['name']}} ---{{$value['text']}}<span style="float:right;">{{$value['time']}}</span></li>
                    @endforeach
                  @endif
                  </div>
                  @if($ticket['bstatus'] == 'Finished')
                  <h3>Internal Notice</h3>
                  <div class='noticeMessages'>
                  @if(isset($comm['notice']))
                    @foreach($comm['notice'] as $key => $value)
                      <li class="cm">{{$value['name']}} ---{{$value['text']}}<span style="float:right;">{{$value['time']}}</span></li>
                    @endforeach
                  @endif
                  </div>
                  ...
                  <div class='noticeBottom'>
                    <form action='#' onSubmit='return false;' id='noticeForm'>
                      
                      <input type='hidden' class='hidden' id='{{$ticket['_id']}}'/>
                      <input type='text' name='noticeText' id='noticeText' value='' placeholder="Internal notice for other IT" />
                      <input type='submit' class="noticeSubmit" name='noticeSubmit' value="post" />
                    </form>
                  </div>
                  @else
                  <div class='chatBottom'>
                    <form action='#' onSubmit='return false;' id='chatForm'>
                      ...
                      <input type='hidden' class='hidden' id='{{$ticket['_id']}}'/>
                      <input type='text' name='text' id='text' value='' placeholder="type your comment" />
